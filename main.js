...
function generateLogs(type, player1, player2, hitValue) {
	...
	switch (type) {
		case 'start':
			...
			break
		case 'hit':
			pattern = logs['hit'][getRandom(logs['hit'].length-1)].replace('[playerKick]', (player2.name)).replace('[playerDefence]', (player1.name));
		...
		case 'defence':
			...
			break
		case 'end':
			pattern = logs['end'][getRandom(logs['end'].length-1)].replace('[playerWins]', (player1.name)).replace('[playerLose]', (player2.name));
			text = `<p>${pattern}</p>`;
			break
		case 'draw':
			pattern = logs['draw'];
